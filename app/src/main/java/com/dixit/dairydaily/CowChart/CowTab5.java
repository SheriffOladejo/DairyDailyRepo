...
public class CowTab5 extends Fragment {

    private OnFragmentInteractionListener mListener;

    private static final String TAG = "CowTab5";
        ...
        if(nameOfSnf.equals("Buffalo")){
            ...
            if(data.getCount() != 0){
                while(data.moveToNext()){
                    list.add(new RateChartModel(data.getString(0), data.getString(5)));
            ...
            list.add(new RateChartModel(data.getColumnName(5), ""));
            if (data.getCount() != 0) {
                while (data.moveToNext()) {
                    list.add(new RateChartModel(data.getString(0), data.getString(5)));
                    //Log.d(TAG, "CowTab1: " + data.getString(0) + " " + data.getString(1));

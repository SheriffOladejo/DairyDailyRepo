...
public class CowTab6 extends Fragment {

    private OnFragmentInteractionListener mListener;
    private static final String TAG = "CowTab6";
        ...
        if(nameOfSnf.equals("Buffalo")){
            ...
            if(data.getCount() != 0){
                while(data.moveToNext()){
                    list.add(new RateChartModel(data.getString(0), data.getString(6)));
            ...
            list.add(new RateChartModel(data.getColumnName(6), ""));
            if (data.getCount() != 0) {
                while (data.moveToNext()) {
                    list.add(new RateChartModel(data.getString(0), data.getString(6)));
                    //Log.d(TAG, "CowTab1: " + data.getString(0) + " " + data.getString(1));

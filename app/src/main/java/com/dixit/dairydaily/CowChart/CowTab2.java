...
public class CowTab2 extends Fragment {

    private OnFragmentInteractionListener mListener;
    private static final String TAG = "CowTab2";
        ...
        if(nameOfSnf.equals("Buffalo")){
            ...
            if(data.getCount() != 0){
                while(data.moveToNext()){
                    list.add(new RateChartModel(data.getString(0), data.getString(2)));
            ...
            list.add(new RateChartModel(data.getColumnName(2), ""));
            if (data.getCount() != 0) {
                while (data.moveToNext()) {
                    list.add(new RateChartModel(data.getString(0), data.getString(2)));
                    //Log.d(TAG, "CowTab1: " + data.getString(0) + " " + data.getString(1));

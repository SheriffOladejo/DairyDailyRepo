...
public class CowTab7 extends Fragment {

    private OnFragmentInteractionListener mListener;
    private static final String TAG = "CowTab7";
        ...
        if(nameOfSnf.equals("Buffalo")){
            ...
            if(data.getCount() != 0){
                while(data.moveToNext()){
                    list.add(new RateChartModel(data.getString(0), data.getString(7)));
            ...
            list.add(new RateChartModel(data.getColumnName(7), ""));
            if (data.getCount() != 0) {
                while (data.moveToNext()) {
                    list.add(new RateChartModel(data.getString(0), data.getString(7)));
                    //Log.d(TAG, "CowTab1: " + data.getString(0) + " " + data.getString(1));

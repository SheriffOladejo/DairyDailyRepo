package com.dixit.dairydaily.Others;

public class Prevalent {
    public static final String remember_me = "remember me";
    public static final String has_account = "has account";
    public static final String selected_device = "Default Device";
    public static String offline_password = "Offline Password";
    public static final String phone_number = "Phone Number";
    public static final String name = "Name";
    public static final String city = "City";
    public static final String state = "State";
    public static final String email = "Email";
    public static String address = "Address";
    public static String expiry_date = "Expiry Date";
    public static String last_update = "Update";
    public static String starter = "starter";
    public static String spark = "spark";
    public static String enterprise = "enterprise";
}
